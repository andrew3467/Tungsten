...
namespace Tungsten
{
    ...
    protected:
        uint32_t LoadTextureData(const std::filesystem::path& path);
    ...
    public:
        static std::shared_ptr<Texture2D> Create(const std::filesystem::path& path) {return std::make_shared<Texture2D>(path);}
